...
const fetchPlanStatusForRedirect = async (): Promise<PlanStatusResponse | null> => {
  try {
...
export const redirectToLogin = async (): Promise<void> => {
  try {
    ...
    if (planStatus && planStatus.isSaas !== undefined) {
      ...
      console.log(`[AuthRedirect] Redirecting to login: ${loginUrl} (isSaas: ${planStatus.isSaas})`);
      window.location.href = loginUrl;
    } else {
      // Fallback to default if plan status is not available
      console.log('[AuthRedirect] Plan status not available, using default redirect');
