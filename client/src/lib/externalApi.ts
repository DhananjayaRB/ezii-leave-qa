...
export async function fetchEmployeeData(): Promise<ExternalEmployee[]> {
  ...
  try {
    console.log('[ExternalAPI] Making request to worker-master-leave endpoint...');
    console.log('[ExternalAPI] Payload:', payload);
    
    const response = await fetch(`${EXTERNAL_API_BASE_URL}/reports/worker-master-leave`, {
      method: 'POST',
      headers: {
    ...
    console.log(`[ExternalAPI] Successfully fetched ${data.data.data.length} employees from external API`);
...
export function transformEmployeeData(externalEmployee: ExternalEmployee) {
  try {
    const transformed = {
      ...
      phoneNumber: externalEmployee.Mobile_number_1 || externalEmployee.Mobile_number_2 || null,
      ...
      user_name: externalEmployee.user_name || 
    ...
    console.error('[ExternalAPI] Error transforming employee data:', error, externalEmployee);
