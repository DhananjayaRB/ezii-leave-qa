        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <Card>
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm text-muted-foreground">Total Granted</p>
                  <p className="text-lg sm:text-2xl font-bold">{analytics.totalGranted}</p>
                </div>
            ...
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg sm:text-2xl font-bold">{analytics.pendingApprovals}</p>
                </div>
                <div className="p-1.5 sm:p-2 bg-yellow-100 rounded-full">
                  <Timer className="h-3 w-3 sm:h-4 sm:w-4 text-yellow-600" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm text-muted-foreground">On Leave Today</p>
                  <p className="text-lg sm:text-2xl font-bold">{analytics.totalOnLeave}</p>
                </div>
                <div className="p-1.5 sm:p-2 bg-green-100 rounded-full">
                  <Plane className="h-3 w-3 sm:h-4 sm:w-4 text-green-600" />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="flex space-x-1 bg-muted p-1 rounded-lg overflow-x-auto">
              {["Leaves", "BTO", "Comp-off"].map((tab) => (
                <button
                  ...
                  className={`flex-1 px-2 sm:px-3 py-2 text-xs sm:text-sm rounded-md transition-colors whitespace-nowrap ${
                    analyticsTab === tab
                      ? 'bg-background text-foreground shadow-sm'
                      : 'text-muted-foreground hover:text-foreground'
                  }`}
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                <h3 className="text-base sm:text-lg font-semibold">Leaves availed: {analytics.totalAvailed}</h3>
              ...
              {(() => {
                ...
                const maxDataValue = Math.max(...monthlyData.map(m => m.total), 1);
                ...
                const labels = [5, 4, 3, 2, 1, 0].map(i => Math.round(step * i));
                

                
                return (
                  <div className="relative w-full overflow-x-auto">
                    <div className="flex items-end justify-between bg-gray-50 p-3 sm:p-6 rounded-lg min-w-[600px] sm:min-w-0 h-[200px] sm:h-[250px]">
                      {/* Y-axis labels */}
                      <div className="flex flex-col justify-between text-xs text-gray-500 mr-1 sm:mr-2 h-[150px] sm:h-[200px]">
                        {labels.map((label, index) => (
                          <span key={index} className="text-xs">{label}</span>
                        ))}
                      </div>
                      
                      {/* Chart bars - RESPONSIVE HEIGHT */}
                      <div className="flex items-end justify-between flex-1 space-x-1 sm:space-x-4 h-[150px] sm:h-[200px]">
                        {monthlyData.map((data, i) => {
                          // Responsive bar height calculation
                          const maxBarHeight = 150; // Fixed for consistency on mobile
                          const barHeightPx = data.total > 0 ? Math.max((data.total / chartMaxValue) * maxBarHeight, 6) : 0;
                            ...
                            <div key={i} className="flex flex-col items-center space-y-1 flex-1 group relative">
                              <div 
                                ...
                                style={{ 
                                  ...
                                  minHeight: data.total > 0 ? '6px' : '2px',
                                  minWidth: '24px',
                              ...
                              <span className="text-xs text-gray-500 transform rotate-45 sm:rotate-0 origin-bottom whitespace-nowrap">
                    ...
                    <div className="flex items-center justify-center flex-wrap gap-2 sm:gap-4 mt-4">
                      {allLeaveTypes.map(leaveType => {
                        ...
                        return (
                          <div key={leaveType} className="flex items-center space-x-1 sm:space-x-2">
                            <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded ${colorClass}`}></div>
                            <span className="text-xs sm:text-sm">{leaveType}</span>
                          </div>
                        );
                      })}
                    </div>
