...
const createBankCompOffFormSchema = (holidays: any[], allowOnNonWorkingDays: boolean = true, maxApplications: number = 999, currentMonthCount: number = 0, userId: string = '') => z.object({
  ...
  message: "Cannot apply comp-off on a holiday. Please select a different start date.",
  path: ["startDate"],
}).refine((data) => {
...
const createAvailCompOffFormSchema = (holidays: any[], allowOnNonWorkingDays: boolean = true, maxApplications: number = 999, currentMonthCount: number = 0, userId: string = '') => z.object({
  ...
  message: "Cannot avail comp-off on a holiday. Please select a different start date.",
  path: ["startDate"],
}).refine((data) => {
  return !isHolidayDate(data.endDate, holidays);
}, {
...
const transferToLeaveFormSchema = z.object({
  ...
  transferDays: z.number().min(0.5, "Minimum 0.5 days required").max(30, "Maximum 30 days allowed"),
  reason: z.string().min(1, "Reason for transfer is required").max(500, "Reason cannot exceed 500 characters"),
});

// Form schema for en-cashing comp-off
const enCashFormSchema = z.object({
  enCashDays: z.number().min(0.5, "Minimum 0.5 days required").max(30, "Maximum 30 days allowed"),
  dailyRate: z.number().min(1, "Daily rate must be greater than 0"),
  reason: z.string().min(1, "Reason for en-cash is required").max(500, "Reason cannot exceed 500 characters"),
});
...
export default function CompensatoryOff() {
  ...
  useEffect(() => {
    ...
    console.log('Comp-off allowOnBehalf permission:', permissions?.permissions?.allowOnBehalf?.compOff);
    console.log('Should show checkbox?', permissions?.permissions?.allowOnBehalf?.compOff === true);
  ...
  useEffect(() => {
    if (!showBankDialog && !showAvailDialog && !showTransferDialog && !showEnCashDialog) {
  ...
  const { data: compOffRecords = [], isLoading, refetch } = useQuery<any[]>({
  ...
  const leaveTypes = allLeaveTypes.filter((leaveType: any) => {
    return employeeAssignments.some((assignment: any) => 
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/comp-off-requests", userId] });
    },
  });
  ...
  const bankedDays = compOffRecords.filter((r: any) => r.type === "bank" && r.status === "approved").length;
  const availedDays = compOffRecords.filter((r: any) => r.type === "avail" && r.status === "approved").length;
  const transferredDays = compOffRecords
    .filter((r: any) => r.type === "transfer" && r.status === "approved")
    .reduce((sum: number, r: any) => sum + (parseFloat(r.transferAmount) || 0), 0);
  ...
  const stats = {
    ...
    pendingApprovals: compOffRecords.filter((r: any) => r.status === "pending").length,
  ...
  const handleTransferToLeaveClick = () => {
    if (!availableActions.includes('transfer')) {
      toast({
        ...
        description: "Transfer to leaves is not enabled in your organization's compensation options",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleTransferToLeave = (data: z.infer<typeof transferToLeaveFormSchema>) => {
    // Use selected employee ID if applying on behalf, otherwise use current user
    const userId = applyOnBehalf && selectedEmployeeId ? selectedEmployeeId : (localStorage.getItem('user_id') || '1');
  ...
  const handleEnCashComp = () => {
    if (!availableActions.includes('encash')) {
      toast({
        title: "Action Not Available",
        description: "En-cash compensation is not enabled in your organization's compensation options",
        variant: "destructive",
      });
      return;
    }
  ...
  const onSubmitBankCompOff = (values: z.infer<typeof bankCompOffFormSchema>) => {
    ...
    let workingDays = calculateWorkingDays(values.startDate, values.endDate, filteredHolidays);
    ...
    console.log('✅ Using dynamic working days calculation for comp-off:', workingDays);
    
    console.log('Bank Comp-off Working Days Calculation:', {
      startDate: startDateStr,
      endDate: endDateStr,
      calculatedWorkingDays: calculateWorkingDays(values.startDate, values.endDate, filteredHolidays),
    ...
    createMutation.mutate(compOffData, {
      ...
      onError: (error: any) => {
        toast({
          title: "Error", 
          description: error.message || "Failed to submit comp-off banking request",
          variant: "destructive",
        });
      },
    });
  ...
  const onSubmitAvailCompOff = (values: z.infer<typeof availCompOffFormSchema>) => {
    // Apply comp-off variant validation rules
    const variant = compOffVariants[0]; // Use first variant or implement variant selection
    ...
    const startDateStr = format(values.startDate, "yyyy-MM-dd");
    const endDateStr = format(values.endDate, "yyyy-MM-dd");
    console.log('✅ Using dynamic working days calculation for avail comp-off:', workingDays);
    ...
    createMutation.mutate(compOffData, {
      ...
      onError: (error: any) => {
        toast({
          title: "Error",
          description: error.message || "Failed to submit comp-off avail request",
          variant: "destructive",
        });
      },
    });
          ...
          <div className="flex flex-col space-y-2 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-3">
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <div className="flex items-center justify-between">
                    <DialogTitle className="text-lg font-semibold">Bank Comp Off</DialogTitle>
                    <Button
                      variant="ghost"
                      size="icon"
                ...
                <Form {...bankForm}>
                  <form onSubmit={bankForm.handleSubmit(onSubmitBankCompOff)} className="space-y-4">
                    {/* Start Date */}
                    <FormField
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                      ...
                      >
                        {createMutation.isPending ? "Banking..." : "Bank Comp-off"}
                      </Button>
                    </div>
                  </form>
                </Form>
            ...
            <Dialog open={showAvailDialog} onOpenChange={setShowAvailDialog}>
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <div className="flex items-center justify-between">
                    <DialogTitle className="text-lg font-semibold">Avail Comp Off</DialogTitle>
                    <Button
                      variant="ghost"
                      size="icon"
                ...
                <Form {...availForm}>
                  <form onSubmit={availForm.handleSubmit(onSubmitAvailCompOff)} className="space-y-4">
                    {/* Start Date */}
                    <FormField
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                      ...
                      >
                        {createMutation.isPending ? "Submitting..." : "Avail Comp-off"}
                      </Button>
                    </div>
                  </form>
                </Form>
            ...
            <Dialog open={showTransferDialog} onOpenChange={setShowTransferDialog}>
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <div className="flex items-center justify-between">
                    <DialogTitle className="text-lg font-semibold">Transfer to Leave</DialogTitle>
                    <Button
                      variant="ghost"
                      size="icon"
                ...
                <Form {...transferForm}>
                  <form onSubmit={transferForm.handleSubmit(handleTransferToLeave)} className="space-y-4">
                    {/* Current Balance Display */}
                    <div className="bg-orange-50 border border-orange-200 rounded-md p-3">
                      <p className="text-sm text-orange-700">
                        Available Comp-off Balance: <strong>{stats.balance} days</strong>
                      </p>
                    </div>
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                      ...
                      >
                        {createMutation.isPending ? "Transferring..." : "Transfer to Leave"}
                      </Button>
                    </div>
                  </form>
                </Form>
            ...
            <Dialog open={showEnCashDialog} onOpenChange={setShowEnCashDialog}>
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <div className="flex items-center justify-between">
                    <DialogTitle className="text-lg font-semibold">En-Cash Comp Off</DialogTitle>
                    <Button
                      variant="ghost"
                      size="icon"
                ...
                <Form {...enCashForm}>
                  <form onSubmit={enCashForm.handleSubmit(onSubmitEnCashComp)} className="space-y-4">
                    {/* Current Balance Display */}
                    ...
                    {enCashForm.watch("enCashDays") && enCashForm.watch("dailyRate") && (
                      <div className="bg-green-50 border border-green-200 rounded-md p-3">
                        <p className="text-sm text-green-700">
                          Total Amount: <strong>₹{(enCashForm.watch("enCashDays") * enCashForm.watch("dailyRate")).toFixed(2)}</strong>
                        </p>
                      </div>
                    )}
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                      ...
                      >
                        {createMutation.isPending ? "Processing..." : "En-Cash Comp-off"}
                      </Button>
                    </div>
                  </form>
                </Form>
          ...
          {availableActions.includes('avail') && (
            ...
            >
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-medium text-teal-700">Avail Comp-off</h3>
                  </div>
          ...
          {availableActions.includes('bank') && (
            ...
            >
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-medium text-blue-700">Bank Comp-off</h3>
                  </div>
          ...
          {availableActions.includes('transfer') && (
            ...
            >
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-medium text-purple-700">Transfer to Leaves</h3>
                  </div>
          ...
          {availableActions.includes('encash') && (
            ...
            >
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-medium text-gray-700">En-Cash Comp</h3>
                  </div>
                        ...
                        <tr key={record.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {record.type === 'transfer' && record.transferAmount 
                              ...
                              : record.dayType === 'full' ? 'Full Day' : 'Half Day'}
                          </td>
