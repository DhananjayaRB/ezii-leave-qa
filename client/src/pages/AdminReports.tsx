...
import { Calendar, Download, Filter, BarChart3, PieChart, TrendingUp, Users, Clock, CheckCircle, XCircle } from "lucide-react";
...
export default function AdminReports() {
  ...
  const { employees: externalEmployees, loading: isLoadingEmployees, error: employeeError } = useExternalEmployeeData();
  ...
  const getEmployeeName = (userId: string) => {
    ...
    const externalEmployee = externalEmployees.find(emp => 
      emp.user_id?.toString() === userId || emp.user_id === parseInt(userId, 10)
    );
  ...
  const mapReportType = (type: string) => {
    switch (type) {
      case "balances": return "balance-report";
  ...
  const [dateRange, setDateRange] = useState<any>({
    from: new Date(2020, 0, 1), // Start from 2020 to capture all historical data
    to: new Date(new Date().getFullYear() + 1, 11, 31) // End next year to capture all future data
  });
  ...
  const getFilteredData = () => {
    ...
    if (reportingManagerData.isReportingManager && reportingManagerData.reportees.length > 0) {
      ...
      console.log(`[AdminReports] Filtered to ${filteredRequests.length} requests and ${filteredEmployees.length} employees for reporting manager`);
    } else {
      console.log(`[AdminReports] Showing all ${filteredRequests.length} requests and ${filteredEmployees.length} employees`);
    }
  ...
  const getLeaveStats = () => {
    ...
    const approvedRequests = allRequests.filter((req: any) => req.status === 'approved').length;
    const pendingRequests = allRequests.filter((req: any) => req.status === 'pending').length;
    const rejectedRequests = allRequests.filter((req: any) => req.status === 'rejected').length;
    ...
    switch (selectedReport) {
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Detailed report of all approved leave applications with comprehensive employee and leave details
              </p>
            </CardHeader>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      ...
                      <TableHead className="font-semibold">From Date: Is Half Day</TableHead>
                      <TableHead className="font-semibold">To Date</TableHead>
                      <TableHead className="font-semibold">To Date: Is Half Day</TableHead>
                      <TableHead className="font-semibold">Total Leave Days</TableHead>
                  ...
                  <TableBody>
                    {(() => {
                      ...
                      const approvedRequests = filteredRequests.filter((req: any) => req.status === 'approved');
                      
                      if (approvedRequests.length === 0) {
                        return (
                          <TableRow>
                            <TableCell colSpan={12} className="py-8 px-4 text-center text-muted-foreground">
                              No approved leave requests found for the selected period
                            </TableCell>
                          </TableRow>
                        );
                      }

                      return approvedRequests.map((request: any, index: number) => {
                        ...
                        const employeeNumber = externalEmployee?.employee_number || request.userId;
                        const employeeName = getEmployeeName(request.userId);
                        const department = externalEmployee?.department || 'N/A';
                        const designation = externalEmployee?.designation || 'N/A';
                        
                        // Format dates
                        const fromDate = request.startDate ? format(new Date(request.startDate), 'dd-MMM-yyyy') : 'N/A';
                        const toDate = request.endDate ? format(new Date(request.endDate), 'dd-MMM-yyyy') : 'N/A';
                        
                        // Determine half day status (assuming half day if working days contains .5)
                        const workingDays = parseFloat(request.workingDays) || 0;
                        const isFromHalfDay = request.fromHalfDay || (workingDays % 1 !== 0 ? 'Yes' : 'No');
                        const isToHalfDay = request.toHalfDay || (workingDays % 1 !== 0 && request.startDate === request.endDate ? 'Yes' : 'No');
                        
                        // Get approved by information (you may need to add this field to your leave requests schema)
                        const approvedBy = request.approvedByName || request.approvedBy || 'System Auto-Approval';
                        
                        return (
                          <TableRow key={`leave-${request.id}-${index}`} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            <TableCell className="font-medium">{employeeNumber}</TableCell>
                            ...
                            <TableCell className="text-center">
                              <Badge variant={isFromHalfDay === 'Yes' ? 'secondary' : 'outline'} className="text-xs">
                                {isFromHalfDay}
                              </Badge>
                            </TableCell>
                            <TableCell>{toDate}</TableCell>
                            <TableCell className="text-center">
                              <Badge variant={isToHalfDay === 'Yes' ? 'secondary' : 'outline'} className="text-xs">
                                {isToHalfDay}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-center">
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-blue-900">
                    {filteredRequests.filter((req: any) => req.status === 'approved').length}
                  </div>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  <div className="text-sm text-green-600 font-medium">Total Days Availed</div>
                ...
                <div className="bg-purple-50 p-4 rounded-lg">
                  <div className="text-sm text-purple-600 font-medium">Unique Employees</div>
                  <div className="text-2xl font-bold text-purple-900">
                    {new Set(filteredRequests
                      .filter((req: any) => req.status === 'approved')
                ...
                <div className="bg-orange-50 p-4 rounded-lg">
                  <div className="text-sm text-orange-600 font-medium">Avg Days per Application</div>
                  <div className="text-2xl font-bold text-orange-900">
                    {(() => {
                      const approvedReqs = filteredRequests.filter((req: any) => req.status === 'approved');
                      const totalDays = approvedReqs.reduce((sum: number, req: any) => sum + (parseFloat(req.workingDays) || 0), 0);
                      return approvedReqs.length > 0 ? (totalDays / approvedReqs.length).toFixed(1) : '0.0';
                    })()}
                  </div>
                </div>
              </div>
        ...
        return (
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Comprehensive report of all withdrawn and rejected leave applications with balance impact analysis
              </p>
            </CardHeader>
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      ...
                      <TableHead className="font-semibold">Total Leave Days</TableHead>
                      <TableHead className="font-semibold">Leave Type</TableHead>
                      <TableHead className="font-semibold">Status Before Action</TableHead>
                      <TableHead className="font-semibold">Balance Before Action</TableHead>
                      <TableHead className="font-semibold">Action</TableHead>
                      <TableHead className="font-semibold">Balance After Action</TableHead>
                      <TableHead className="font-semibold">Action Initiated By</TableHead>
                  ...
                  <TableBody>
                    {(() => {
                      ...
                      const targetRequests = filteredRequests.filter((req: any) => 
                      ...
                      if (targetRequests.length === 0) {
                        return (
                          <TableRow>
                            <TableCell colSpan={15} className="py-8 px-4 text-center text-muted-foreground">
                              No withdrawn or rejected leave requests found for the selected period
                            </TableCell>
                          </TableRow>
                        );
                      }

                      return targetRequests.map((request: any, index: number) => {
                        // Get employee details from external API
                        const externalEmployee = externalEmployees.find(emp => 
                          emp.user_id?.toString() === request.userId || 
                          emp.user_id === parseInt(request.userId, 10)
                        ...
                        // Format dates
                        const applicationDate = request.createdAt ? format(new Date(request.createdAt), 'dd-MMM-yyyy') : 'N/A';
                        const fromDate = request.startDate ? format(new Date(request.startDate), 'dd-MMM-yyyy') : 'N/A';
                        const toDate = request.endDate ? format(new Date(request.endDate), 'dd-MMM-yyyy') : 'N/A';
                        const leavePeriod = fromDate === toDate ? fromDate : `${fromDate} to ${toDate}`;
                        ...
                        let reason = request.rejectionReason || request.withdrawalReason || 'No reason provided';
                        
                        switch (request.status) {
                          case 'rejected':
                            statusBeforeAction = 'pending';
                            actionTaken = 'Rejected';
                            actionInitiatedBy = request.rejectedBy || request.actionBy || 'Admin';
                            reason = request.rejectionReason || 'No reason provided';
                            break;
                          case 'withdrawn':
                            statusBeforeAction = request.previousStatus || 'pending';
                            actionTaken = 'Withdrawn';
                            actionInitiatedBy = request.withdrawnBy || employeeName || 'Employee';
                            reason = request.withdrawalReason || 'Employee initiated withdrawal';
                            break;
                          case 'withdrawal_pending':
                            statusBeforeAction = 'approved';
                            actionTaken = 'Withdrawal Requested';
                            actionInitiatedBy = request.withdrawnBy || employeeName || 'Employee';
                            reason = request.withdrawalReason || 'Employee requested withdrawal';
                            break;
                          case 'withdrawal_approved':
                            statusBeforeAction = 'approved';
                            actionTaken = 'Withdrawal Approved';
                            actionInitiatedBy = request.withdrawalApprovedBy || 'Admin';
                            reason = request.withdrawalReason || 'Withdrawal approved by admin';
                            break;
                        }
                        ...
                        } else if (request.status === 'withdrawn' && statusBeforeAction === 'pending') {
                          balanceBeforeAction = 'No impact';
                          balanceAfterAction = 'No impact';
                        } else if (request.status === 'withdrawn' || request.status === 'withdrawal_approved') {
                        ...
                        const getStatusBadgeVariant = (status: string) => {
                          switch (status) {
                            ...
                            case 'withdrawal_pending': return 'outline';
                            case 'withdrawal_approved': return 'default';
                            default: return 'outline';
                          }
                        };
                        
                        return (
                          <TableRow key={`withdraw-reject-${request.id}-${index}`} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            <TableCell className="font-medium">{employeeNumber}</TableCell>
                            <TableCell>{employeeName}</TableCell>
                            <TableCell>{department}</TableCell>
                            <TableCell>{designation}</TableCell>
                            ...
                            <TableCell className="text-sm">{balanceBeforeAction}</TableCell>
                            <TableCell>
                              <Badge variant={getStatusBadgeVariant(request.status)} className="capitalize">
                                {actionTaken}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-sm">{balanceAfterAction}</TableCell>
                            <TableCell>{actionInitiatedBy}</TableCell>
                            <TableCell className="max-w-xs">
                              <div className="text-sm text-gray-600 truncate" title={reason}>
                                {reason}
                              </div>
                            </TableCell>
                          </TableRow>
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-red-50 p-4 rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-red-900">
                    {filteredRequests.filter((req: any) => req.status === 'rejected').length}
                  </div>
                </div>
                <div className="bg-orange-50 p-4 rounded-lg">
                  <div className="text-sm text-orange-600 font-medium">Total Withdrawn</div>
                  <div className="text-2xl font-bold text-orange-900">
                    {filteredRequests.filter((req: any) => 
                      req.status === 'withdrawn' || req.status === 'withdrawal_approved'
                    ).length}
                  </div>
                </div>
                <div className="bg-yellow-50 p-4 rounded-lg">
                  <div className="text-sm text-yellow-600 font-medium">Withdrawal Pending</div>
                  <div className="text-2xl font-bold text-yellow-900">
                    {filteredRequests.filter((req: any) => req.status === 'withdrawal_pending').length}
                  </div>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg">
                  <div className="text-sm text-purple-600 font-medium">Days Affected</div>
                  <div className="text-2xl font-bold text-purple-900">
                    {filteredRequests
                      .filter((req: any) => 
                        req.status === 'rejected' || 
      ...
      case "collaborative-leave-report":
        ...
        const getFilteredCollaborativeTasks = () => {
          ...
          console.log('🔍 [COLLABORATIVE TASKS DEBUG] Raw tasks from API:', {
            totalTasks: filtered.length,
            rawData: filtered.slice(0, 3).map(t => ({
              id: t.id,
              status: t.status,
          ...
          // Skip date filtering for collaborative tasks - show all tasks by default
          console.log('📅 [DATE FILTER] Skipping date filter for collaborative tasks to show all available data');

          // Filter by task status
          if (selectedTaskStatus !== "all") {
            const beforeStatusFilter = filtered.length;
            filtered = filtered.filter((task: any) => task.status === selectedTaskStatus);
            console.log(`📊 [STATUS FILTER] Filtered ${beforeStatusFilter} → ${filtered.length} tasks for status: ${selectedTaskStatus}`);
          }

          // Filter by assignee name
          if (taskAssigneeFilter.trim()) {
            const beforeAssigneeFilter = filtered.length;
            filtered = filtered.filter((task: any) => 
              task.assigneeName?.toLowerCase().includes(taskAssigneeFilter.toLowerCase()) ||
              task.assigneeEmail?.toLowerCase().includes(taskAssigneeFilter.toLowerCase())
            );
            console.log(`👤 [ASSIGNEE FILTER] Filtered ${beforeAssigneeFilter} → ${filtered.length} tasks for filter: ${taskAssigneeFilter}`);
          }
        ...
        return (
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Track all collaborative leave task assignments across the organization with current statuses and performance metrics
              </p>
            </CardHeader>
            <CardContent className="p-0">
              {/* Summary Statistics */}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-white p-4 rounded-lg border">
                    ...
                    <p className="text-2xl font-bold text-green-700">
                      {filteredTasks.filter((task: any) => task.status === 'done').length}
                    </p>
                  </div>
                  <div className="bg-white p-4 rounded-lg border">
                    <div className="flex items-center gap-2">
                    ...
                    <p className="text-2xl font-bold text-yellow-700">
                      {filteredTasks.filter((task: any) => ['pending', 'accepted'].includes(task.status)).length}
                    </p>
                  </div>
                  <div className="bg-white p-4 rounded-lg border">
                    <div className="flex items-center gap-2">
                    ...
                    <p className="text-2xl font-bold text-red-700">
                      {filteredTasks.filter((task: any) => ['rejected', 'not_done'].includes(task.status)).length}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      ...
                      <TableHead className="font-semibold">Leave Request ID</TableHead>
                      <TableHead className="font-semibold">Assigned By</TableHead>
                      <TableHead className="font-semibold">Assignee Name</TableHead>
                      <TableHead className="font-semibold">Assignee Email</TableHead>
                      <TableHead className="font-semibold">Task Description</TableHead>
                      <TableHead className="font-semibold">Support Date</TableHead>
                      <TableHead className="font-semibold">Status</TableHead>
                      <TableHead className="font-semibold">Accepted At</TableHead>
                      <TableHead className="font-semibold">Last Updated</TableHead>
                      <TableHead className="font-semibold">Status Comments</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredTasks.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={11} className="text-center py-8 text-gray-500">
                          No collaborative leave tasks found for the selected criteria.
                        </TableCell>
                      </TableRow>
                    ) : (
                      filteredTasks.map((task: any, index: number) => {
                        const getStatusBadge = (status: string) => {
                          switch (status) {
                            ...
                            case 'not_done':
                              return <Badge className="bg-gray-100 text-gray-800 border-gray-200">Not Done</Badge>;
                            default:
                              return <Badge variant="outline">{status}</Badge>;
                          }
                        };

                        const supportDateRange = task.expectedSupportDateFrom && task.expectedSupportDateTo 
                          ...
                          : task.expectedSupportDate 
                            ? format(new Date(task.expectedSupportDate), 'dd-MMM-yyyy')
                            : 'Not specified';

                        return (
                          <TableRow key={`task-${task.id}-${index}`} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            ...
                            <TableCell className="font-medium">
                              {getEmployeeName(task.leaveRequesterId) || 'Unknown'}
                            </TableCell>
                            <TableCell className="font-medium">{task.assigneeName}</TableCell>
                            <TableCell className="text-sm text-gray-600">{task.assigneeEmail}</TableCell>
                            <TableCell className="max-w-xs">
                              <div className="truncate" title={task.taskDescription}>
                                {task.taskDescription}
                              </div>
                            </TableCell>
                            <TableCell className="text-sm">{supportDateRange}</TableCell>
                            <TableCell>{getStatusBadge(task.status)}</TableCell>
                            <TableCell className="text-sm">
                              {task.acceptedAt ? format(new Date(task.acceptedAt), 'dd-MMM-yyyy HH:mm') : 'Not accepted'}
                            </TableCell>
                            <TableCell className="text-sm">
                              {task.lastStatusUpdate ? format(new Date(task.lastStatusUpdate), 'dd-MMM-yyyy HH:mm') : 'No updates'}
                            </TableCell>
                            <TableCell className="max-w-xs">
                              <div className="truncate" title={task.statusComments || 'No comments'}>
