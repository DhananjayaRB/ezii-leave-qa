...
export default function EmployeeReports() {
  ...
  const getCurrentEmployeeName = () => {
    ...
    const externalEmployee = externalEmployees.find(
      (emp) =>
        ...
        emp.user_id === parseInt(currentUserId, 10),
    );
    ...
    return availableLeaveVariants.map((variant: any) => {
      ...
      const matchingRequests = requestsArray.filter(
        (req: any) =>
          ...
          ((req.leaveVariantId === null || req.leaveVariantId === undefined) &&
  ...
  const getFilteredRequests = () => {
    ...
    if (selectedLeaveType !== "all") {
      filtered = filtered.filter((req: any) => {
        const leaveType = (leaveTypes as any[]).find(
          (type: any) => type.id === req.leaveTypeId,
        );
  ...
  const getWithdrawalRequests = () => {
    return (myLeaveRequests as any[]).filter(
      (req: any) =>
  ...
  const exportData = () => {
    const dataToExport = {
      ...
      dateRange: dateRange,
      data:
        selectedReport === "my-leave-balances"
          ? myBalances
          : selectedReport === "my-withdrawal-history"
            ? withdrawalRequests
            : filteredRequests,
    };
          ...
          <Table>
            ...
            <TableBody>
              {filteredRequests.map((request: any) => {
                const leaveType = (leaveTypes as any[]).find(
                  (type: any) => type.id === request.leaveTypeId,
                );
                return (
                  <TableRow key={request.id}>
                    <TableCell>
                      {format(
                        new Date(request.appliedDate || request.createdAt),
                        "MMM dd, yyyy",
                      )}
                    </TableCell>
        ...
        <CardContent>
          <div className="flex justify-between items-center mb-6">
            <p className="text-muted-foreground">
              Employee:{" "}
              <span className="font-medium">{getCurrentEmployeeName()}</span> |
                  ...
                  <TableRow key={request.id}>
                    ...
                    <TableCell>
                      {request.withdrawalDate
                        ? format(
                            new Date(request.withdrawalDate),
                            "MMM dd, yyyy",
                          )
                        : "Pending"}
                    </TableCell>
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant={
                selectedReport === "my-leave-balances" ? "default" : "outline"
              }
