...
export default function Holidays() {
  ...
  const { workPattern, holidays: workPatternHolidays, isLoading: workPatternLoading } = useWorkPattern();
  ...
  const { data: allHolidaysData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('🌐 [External Holidays] Calling API: https://qa-api.resolveindia.com/organization/holidays');
  ...
  // Use external holidays if available, fallback to database holidays
  console.log('🔄 Holiday Source: Using', allHolidaysData && allHolidaysData.length > 0 ? 'External API' : 'Database', 'holidays');
  console.log('🏢 [Holidays Debug] All holidays count:', allHolidays.length);
  console.log('🏢 [Holidays Debug] Raw holidays:', allHolidays);
  console.log('🏢 [Holidays Debug] Work pattern selectedHolidays:', workPattern?.selectedHolidays);
  console.log('🏢 [Holidays Debug] Filtered holidays count:', filteredHolidays.length);
  ...
  const generateCalendarDays = () => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      if (selectedMonth.includes('August') && currentDate.getMonth() === 7 && currentDate.getDate() >= 14 && currentDate.getDate() <= 17) {
        console.log(`🔍 August Debug: ${dateString} -> Day ${currentDate.getDate()}, Holiday: ${holiday?.name || 'none'}`);
      }
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-center min-h-[400px]">
              <div className="text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">Failed to load holidays</h3>
                <p className="text-gray-600 mb-4">
                  {error instanceof Error ? error.message : 'Unable to fetch holiday data from the server'}
                </p>
                <Button 
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-semibold text-gray-900">
              Holidays {filteredHolidays && filteredHolidays.length > 0 && `(${filteredHolidays.length} total)`}
            </h1>
          ...
          {viewMode === "monthly" ? (
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-semibold text-gray-900">{selectedMonth}</h3>
            ...
            <Card>
              <CardContent className="p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-6 text-center">2025 Holiday Calendar</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {yearlyCalendar.map((month) => (
                    <div key={month.monthName} className="bg-white border rounded-lg p-4">
                      {/* Month Header */}
                      ...
                      <div className="mt-3 space-y-1">
                        ...
                        {month.days.filter(day => day.isHoliday && day.isCurrentMonth).length === 0 && (
                          <div className="text-xs text-gray-400 italic">No holidays</div>
                        )}
                      </div>
                    </div>
                  ))}
