...
export default function CompOffSetup({
  ...
  const [variantAssignments, setVariantAssignments] = useState<
    Record<string, any[]>
  >({});
  const [currentVariantForAssignment, setCurrentVariantForAssignment] =
    useState<any>(null);
  ...
  useEffect(() => {
    const loadEmployees = async () => {
      try {
        ...
        console.log(
          "CompOffSetup: External API loaded employees:",
          transformedEmployees.length,
        );
        setAllEmployees(transformedEmployees);
      } catch (error) {
        console.error(
          "CompOffSetup: Error loading employees from external API:",
          error,
        );
        ...
        for (const variant of compOffVariants) {
          try {
            const response = await fetch(
              `/api/employee-assignments/comp-off-variant/${variant.id}`,
              {
                credentials: "include",
              },
            );
            if (response.ok) {
              ...
              console.log(
                `CompOff assignments for variant ${variant.id}:`,
                variantAssignments,
              );
              if (
                ...
                console.log(
                  `CompOff assigned user IDs for variant ${variant.id}:`,
                  assignedUserIds,
                );
                ...
                const assignedEmployeeData = allEmployees.filter((emp) => {
                  ...
                  const isAssigned =
                    assignedUserIds.includes(empId) ||
                    assignedUserIds.includes(empId?.toString());
                  if (isAssigned) {
                    console.log(
                      `CompOff employee matched: ${emp.user_name || emp.name} (ID: ${empId})`,
                    );
                  }
                  return isAssigned;
                });
                ...
                console.log(
                  `CompOff final assignments for variant ${variant.id}:`,
            ...
            console.error(
              `Error loading assignments for variant ${variant.id}:`,
              error,
            );
          }
        }
  ...
  const toggleCompOffMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: compOffEnabled
          ? "Comp off enabled successfully."
          : "Comp off disabled successfully.",
      });
    },
    onError: (error) => {
      toast({
        ...
        <div className="mb-8">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-gray-900">
                      {variant.name || variant.leaveVariantName}
                    </h4>
                    <p className="text-sm text-gray-500">
                      Applied to {variantAssignments[variant.id]?.length || 0}{" "}
                      Employee
                      {(variantAssignments[variant.id]?.length || 0) !== 1
                        ? "s"
                        : ""}
                    </p>
                  </div>
          ...
          {compOffVariants.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">
                Click "Create Comp-off variant" to get started.
              </p>
            </div>
          )}
