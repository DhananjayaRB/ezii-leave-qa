...
export default function ImportLeaveDataSetup({ onNext, onPrevious, isLast, isLoading }: ImportLeaveDataSetupProps) {
  ...
  const validateFileMutation = useMutation({
    ...
    onSuccess: (response: any) => {
      ...
      if (response.errors.length === 0) {
        ...
        toast({
          ...
          description: `Found ${response.errors.length} validation errors. Please review and fix them.`,
          variant: "destructive",
        });
      }
    },
    onError: (error: Error) => {
      ...
      toast({
        ...
        description: "Failed to validate file. Please check the file format and try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const importDataMutation = useMutation({
    mutationFn: async () => {
      ...
      if (jwtToken) {
        ...
        console.log(`[ImportSetup] ✅ Passing JWT token for ${importType} import`);
      } else {
  ...
  // Download template
  const handleDownloadTemplate = async (templateType: "balances" | "transactions") => {
    try {
      ...
      if (templateType === "balances") {
        ...
        ws['!cols'] = [
          ...
          { wch: 20 }, // EmpName
          { wch: 15 }, // LeaveType
          { wch: 18 }, // LeaveTakenStartDate
          { wch: 20 }, // Is Start Date a Half Day
      ...
      if (templateType === "balances") {
        ...
        ws['!cols'] = [
          ...
          { wch: 20 }, // EmpName
          { wch: 15 }, // LeaveType
          { wch: 20 }, // LeaveOpeningBalance
          { wch: 15 }, // LeaveEncashed
          { wch: 12 }  // LeaveLapsed
        ];

        // Style the header row
        if (!ws['!merges']) ws['!merges'] = [];
        ws['!merges'].push({ s: { r: 0, c: 0 }, e: { r: 0, c: 5 } }); // Merge title row (6 columns)
      } else {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `leave_${templateType}_import_template.xlsx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      ...
      toast({
        title: "Template Downloaded",
        description: `Successfully downloaded ${templateType} template with ${templateData.length - 3} employee records.`,
      });
  ...
  const importData = () => {
    if (!isValidated || validationErrors.length > 0) {
      toast({
        ...
        description: "Please validate the file and fix all errors before importing.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold mb-2">Import Leave Data</h2>
        <p className="text-gray-600">
          Upload employee leave balance or transaction data to import into the system.
        </p>
      </div>
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <Button
                variant="outline"
                onClick={() => handleDownloadTemplate("balances")}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {previewData.length > 0 && (
              <div>
                <h4 className="font-medium mb-2">Data Preview ({previewData.length} records)</h4>
