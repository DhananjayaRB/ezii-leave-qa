...
import { ChevronLeft, ChevronRight, Calendar, Clock, CheckCircle, XCircle, TrendingUp, BarChart3, FileText } from 'lucide-react';
...
export default function NewEmployeeOverview() {
  ...
  const userAssignments = Array.isArray(allAssignments) ? 
    allAssignments.filter((assignment: any) => assignment.userId === currentUserId) : [];
  ...
  const availableLeaveVariants = Array.isArray(leaveVariantsData) ? 
    leaveVariantsData.filter((variant: any) => assignedVariantIds.includes(variant.id)) : [];
  ...
  const calculateSophisticatedMetrics = () => {
    ...
    console.log('🎯 [Dashboard] Using SOPHISTICATED calculation (same as Leave Applications):', {
      currentUserId,
    ...
    availableLeaveVariants.forEach((variant: any) => {
      ...
      console.log(`🔍 [Dashboard CALCULATION] Dynamic calculation for ${variant.leaveTypeName}:`, {
    ...
    const totalCurrentBalance = balancesArray.reduce((sum: number, balance: any) => 
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col space-y-4 sm:flex-row sm:justify-between sm:items-start sm:space-y-0">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Employee Overview</h1>
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-1">Your leave analytics and applications dashboard</p>
        </div>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1 sm:space-x-2">
                ...
                <span className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Total Leaves</span>
              </div>
            </div>
            <div className="mt-2">
              <span className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">{totalEntitlement.toFixed(1)}</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1 sm:space-x-2">
                ...
                <span className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Total Availed</span>
              </div>
            </div>
            <div className="mt-2">
              <span className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">{totalAvailed.toFixed(1)}</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1 sm:space-x-2">
                ...
                <span className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Pending Approvals</span>
              </div>
            </div>
            <div className="mt-2">
              <span className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">{pendingCount}</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1 sm:space-x-2">
                ...
                <span className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Balance</span>
              </div>
            </div>
            <div className="mt-2">
              <span className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">{totalBalance.toFixed(1)}</span>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent>
            <Tabs defaultValue="leaves" className="w-full">
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger value="leaves" className="text-xs sm:text-sm">Leaves ({leaveRequests.length})</TabsTrigger>
                <TabsTrigger value="bto" className="text-xs sm:text-sm">BTO ({ptoRequests.length})</TabsTrigger>
                <TabsTrigger value="comp-off" className="text-xs sm:text-sm">Comp-off ({compOffRequests.length})</TabsTrigger>
              </TabsList>
                ...
                {ptoRequests.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {ptoRequests.slice(0, 5).map((request: any) => (
                      <div key={request.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <div>
                          <p className="font-medium text-gray-900 dark:text-white">BTO Request</p>
                          <p className="text-sm text-gray-500 dark:text-gray-400">
                ...
                {compOffRequests.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {compOffRequests.slice(0, 5).map((request: any) => (
                      <div key={request.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <div>
                          <p className="font-medium text-gray-900 dark:text-white">Comp-off Request</p>
                          <p className="text-sm text-gray-500 dark:text-gray-400">
          ...
          <CardContent>
            <div className="grid grid-cols-7 gap-1 text-center">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                <div key={day} className="p-1 sm:p-2 text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">
                  {day.substring(0, 3)}
                </div>
              ))}
              {(() => {
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                  ...
                  const pendingOnDate = leaveRequests.filter((req: any) => req.status === 'pending').some(leave => {
                    const startDate = new Date(leave.startDate);
                    const endDate = new Date(leave.endDate);
                    return currentDate >= startDate && currentDate <= endDate;
                  });
                  ...
                  const rejectedOnDate = leaveRequests.filter((req: any) => req.status === 'rejected').some(leave => {
                    const startDate = new Date(leave.startDate);
                    const endDate = new Date(leave.endDate);
                    return currentDate >= startDate && currentDate <= endDate;
                  });
                  ...
                  calendarDays.push(
                    <div 
                      key={day} 
                      className={`p-1 sm:p-2 text-xs sm:text-sm relative cursor-pointer ${statusClass} min-h-[32px] sm:min-h-[40px]`}
                    >
                      {day}
                      {(approvedOnDate || pendingOnDate || rejectedOnDate) && (
                        <div className={`absolute bottom-0 left-0.5 right-0.5 h-0.5 sm:h-1 ${indicatorColor} rounded-full`}></div>
                      )}
                    </div>
                  );
                }
            ...
            <div className="flex items-center justify-center mt-4 space-x-2 sm:space-x-4">
              <div className="flex items-center space-x-1">
                ...
                <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Approved</span>
              </div>
              <div className="flex items-center space-x-1">
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Rejected</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg font-semibold">Leave Usage Trends</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {/* Monthly Breakdown */}
          <div className="mt-6">
            <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-3">Monthly Leave Breakdown</h4>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {trendsData
                .filter(month => month.leaves > 0)
                .map((month) => (
                  <div key={month.month} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
